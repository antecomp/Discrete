...
e.g $P(x)$ represents some predicate such as "x is a perfect square". \smallbreak
...
\begin{tabular}{|p{1in} | p{2in} | p{2in} | p{1.5in} |}
	\hline
	\rowcolor{lightgray} Predicate & True When... & False When... & Examples \\
	\hline \\
	$\forall x P(x)$ & If P(x) is true for \textbf{every} x in the domain & If there is \textbf{any} x in the domain such that P(x) is false &  AGH
	
\end{tabular}



\todo[inline]{TODO: make this table not look terrible (I hate \LaTeX)}



% once we sub in the vars, it becomes a prop, P(x) is a Pred, P(5) is a (false) proposition.
% for the truth values with quantsm right the false as "if theres ANY x in the domain (at least one) that makes p(x) false" (first 1)
