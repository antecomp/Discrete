...
\begin{enumerate}
	...
	\begin{itemize}[label={\faAngleRight}]
		\item $\forall x \ P(x) \implies P(a)$, where $a$ is a constant \textcolor{green}{\checkmark}
...
\begin{itemize}[label={}, leftmargin=0.3cm]
	...
	\begin{itemize}[label={}]
		\item For example, in the integer domain, if $P(x,y)$ means "$ y > x $" then $\forall x \ \exists y \ P(x,y)$ is true, but $\exists y \ P(y,y)$ is false.
	\end{itemize}
	\item \cul{Example}
	\begin{itemize}[label={}]
