...
\bigbreak
\define[An argument is \textbf{valid} if and only if \textbf{its conclusion is never false while its premises are true.}]
\intextnote[We can't use a truth table to validate an argument since it only shows the truth values for the statement as a whole, instead we need to use new \textbf{Inference Rules}]
...
\begin{enumerate}
	...
	\item Think backwards. Start from what you want and then seek supporting premises, current results, and necessary equivalence laws and inference rules, until you reach the given premises.
...
This one is already in its standard form - so we just need to identify each part of the standard $P_1 \land P_2 \land P_3 ... \land P_n \implies Q$ form. At the end of this we want to prove that B is true.
...
\begin{tabular}{r  l  l  l  l  l}
	...
	$(A \implies B)$ & $\land$ & \multicolumn{3}{c}{\Longunderstack{$A$ \\ \tiny also by first inf rule}} & $\downarrow$ \\
...
\begin{enumerate}
	...
	\item $B$
	\item $A \land B$ \hspace*{3cm} (1,4)
	\item $D \lor \land C$ \hspace*{3cm} (3,5)
	\item $C \implies D$ \hspace*{3cm} (6, Commutative, Implication) - {\tiny Communicative used to swap C, D}
\end{enumerate}
\todo[inline]{TODO: ADD MORE NOTES TO THIS ONE, I GENIUNELY AM NOT FOLLOWING THE PROCESS HERE}


% ex1 already in the correct form
